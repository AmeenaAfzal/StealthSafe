...
export default function WeatherScreen({ onUnlock, settings }) {
  ...
  return (
    <div className="relative bg-gradient-to-b from-sky-400 via-blue-500 to-blue-700 text-white overflow-y-auto scroll-thin" style={{ height: '100%' }}>
      ...
      <div className="relative mx-5 mb-4 mt-2">
        <input type="text" value={searchVal} onChange={e => setSearchVal(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') checkSafeword(searchVal) }} placeholder={`📍 ${CITY}`} className="w-full bg-white/20 backdrop-blur rounded-full px-4 py-2.5 text-sm text-white placeholder-white/60 outline-none pr-9" />
        <button onClick={() => setShowSW(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/50 hover:text-white text-xs font-bold">?</button>
      </div>

      <AnimatePresence>
        {showSW && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="mx-5 mb-3 bg-black/60 backdrop-blur rounded-2xl p-4 text-xs z-10">
            <p className="font-semibold mb-2 text-white">Safewords (type + Enter):</p>
            {settings.safewords.map((sw, i) => sw.word ? (
              <p key={i} className="text-white/70 mb-1"><span className="font-mono text-white">{sw.word}</span> {' → '} {{ checkin: '✅ Check-in', sos: '🚨 SOS', deadman: '⏱ Dead Man\'s Switch' }[sw.action]}</p>
            ) : null)}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-start justify-between px-7 py-2">
        ...
        <div className="flex flex-col gap-5 items-center pt-2">
          <button onClick={handleSunTap} className="relative focus:outline-none active:scale-95 transition-transform">
            <Sun size={52} strokeWidth={1.5} className={`transition-all duration-150 ${sunPrimed ? 'text-yellow-300 drop-shadow-lg scale-110' : 'text-white/90'}`} />
          ...
          <button onClick={handleCloudTap} className="focus:outline-none active:scale-95 transition-transform">
            <Cloud size={38} strokeWidth={1.5} className="text-white/90" />
          </button>
        </div>
      </div>

      <div className="grid grid-cols-4 gap-2 mx-5 mt-4">
        {[ { icon: <Droplets size={15} />, label: 'Humidity', value: `${HUMIDITY}%` }, { icon: <Wind size={15} />, label: 'Wind', value: `${WIND_KMH} km/h` }, { icon: <Thermometer size={15} />, label: 'UV', value: `${UV_INDEX}` }, { icon: <Eye size={15} />, label: 'Visibility', value: `${VISIBILITY_KM} km` } ].map(({ icon, label, value }) => (
      ...
      <div className="mx-5 mt-4 bg-white/10 backdrop-blur rounded-3xl p-4">
        ...
        <ResponsiveContainer width="100%" height={90}>
          <AreaChart data={HOURLY}>
            ...
            <Tooltip contentStyle={{ background: 'rgba(0,0,0,0.5)', border: 'none', borderRadius: 8, fontSize: 10 }} itemStyle={{ color: '#fff' }} />
            <Area type="monotone" dataKey="temp" stroke="#fff" strokeWidth={2} fill="#fff" fillOpacity={0.2} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
        ...
        <div className="space-y-2.5">
          {WEEK.map(({ day, icon, hi, lo }) => (
            <div key={day} className="flex items-center justify-between text-sm">
              ...
              <div className="flex gap-3"><span className="font-medium">{hi}°</span><span className="opacity-40">{lo}°</span></div>
            </div>
          ))}
        </div>
      </div>
